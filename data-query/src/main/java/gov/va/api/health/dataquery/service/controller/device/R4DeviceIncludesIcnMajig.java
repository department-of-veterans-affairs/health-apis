...
/**
 * Intercept all RequestMapping payloads of Type Device.class or Bundle.class. Extract ICN(s) from
 * these payloads with the provided function. This will lead to populating the X-VA-INCLUDES-ICN
 * header.
 */
@ControllerAdvice
public class R4DeviceIncludesIcnMajig implements ResponseBodyAdvice<Object> {
  @Delegate
  private final ResponseBodyAdvice<Object> delegate =
      IncludesIcnMajig.<Device, Device.Bundle>builder()
          .type(Device.class)
          .bundleType(Device.Bundle.class)
          .extractResources(bundle -> bundle.entry().stream().map(AbstractEntry::resource))
          .extractIcns(resource -> Stream.ofNullable(asReferenceId(resource.patient())))
          .build();
}
