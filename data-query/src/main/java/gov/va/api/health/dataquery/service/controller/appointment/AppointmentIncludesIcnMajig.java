...
/**
 * Intercept all RequestMapping payloads of Type Appointment.class or Bundle.class. Extract
...
public class AppointmentIncludesIcnMajig
        ...
        Appointment, Appointment.Entry, Appointment.Bundle> {
    ...
    public AppointmentIncludesIcnMajig() {
        super(
                ...
                body -> Stream.ofNullable(body.participant()==null||body.participant().isEmpty()?null:body.participant().stream()
                        .map(p->p.actor())
                        .filter(r->r.reference().contains("Patient"))
                        .map(i->Transformers.asReferenceId(i)).collect(Collectors.joining(","))));
    }
}
